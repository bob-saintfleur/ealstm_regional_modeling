...
from neuralhydrology.evaluation.evaluate import start_evaluation, start_eval_climato
...
def _get_args() -> dict:
    ...
    parser.add_argument('mode', choices=["train", "continue_training", "climatology", "finetune",
                                         "evaluate"])
    ...
    if (args["mode"] == "climatology") and (args["run_dir"] is None):
        raise ValueError("Missing path to run directory")
...
def _main():
    ...
    if args["mode"] == "train":
        ...
        eval_on_clim(run_dir=Path(args["run_dir"]), period=args["period"], epoch=args["epoch"], gpu=args["gpu"])
    else:
...
def eval_on_clim(run_dir: Path, period: str, epoch: int = None, gpu: int = None):
    """Start evaluating a trained model.
    ...
    start_eval_climato(cfg=config, run_dir=run_dir, epoch=epoch, period=period)
